...
from iii_layer import IIILayer #

utils.make_deterministic(123)

# remove rgb normalize
def setup_args():
    ...
    parser.add_argument('--backbone', type=str, choices=['resnet50','resnet34'],
                        default="resnet50_gn", help="backbone cnns")
    parser.add_argument('--resume', type=str, default=None,
                        help="checkpoint to resume")

    parser.add_argument('--saveroot',  default="./experiments/addiiilayer2/",
...
def train_one_epoch(args, dataloader, model, model_pre, criterion, optimizer, optimizer_pre,accuracy=accuracy, device=None, mixup_fn=None):
    since = time.time()
    if device is None:
...
def evaluate(dataloader, model, model_pre, criterion, accuracy, device=None):
    if device is None:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
def main(args):
    ...
    if args.resume is not None:
        model, model_pre = utils.resume_model_all(model, model_pre, args.resume, state_dict_key="model", state_dict_key_pre="model_pre")
    ...
    if args.epochs > 0:
        ...
        optimizer_pre = torch.optim.AdamW(
            model_pre.parameters(), lr=args.lr, weight_decay=args.decay)
        ...
        lr_scheduler_pre = torch.optim.lr_scheduler.LambdaLR(
            optimizer_pre, lambda epoch: lrs[epoch-1], verbose=True)
        log["train"] = []
        log["lr"] = []
        print(model)
    for epoch in range(args.epochs):
        print("epoch: %d --starts from 0 and ends at %d" %
              (epoch, args.epochs-1))
        loss, acc = train_one_epoch(args, dataloader_dict["train"], model, model_pre, criterion_train,
                                    optimizer, optimizer_pre, accuracy=accuracy, device=device, mixup_fn=mixup_fn)
    ...
    if args.epochs > 0:
        ...
        utils.save_checkpoint_all(save_path, model, model_pre, key="model", key_pre="model_pre")

    # testing
    print("test started")
    loss, acc = evaluate(
        dataloader_dict["test"], model, model_pre, criterion, accuracy=accuracy, device=device)
