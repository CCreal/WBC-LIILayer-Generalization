...
def setup_savedir(prefix="", basedir="./experiments", args=None, append_args=[], add_time=False):
    savedir = prefix
...
def resume_model_all(model, model_pre, resume, state_dict_key="model", state_dict_key_pre="model_pre"):
    '''
    model:pytorch model
    resume: path to the resume file
    state_dict_key: dict key
    ...
    if state_dict_key_pre is not None:
        pretrained_dict = checkpoint[state_dict_key_pre]
    else:
        assert 1==2
...
def save_checkpoint_all(path, model, model_pre, key="model", key_pre="model_pre"):
    # save model state dict
    checkpoint = {}
    checkpoint[key] = model.state_dict()
    checkpoint[key_pre] = model_pre.state_dict()
    torch.save(checkpoint, path)
    print("checkpoint saved at", path)
